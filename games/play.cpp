...
// Last update Fri Apr  7 17:12:08 2017 ilyas semmaoui
//
...
void	where_am_i(arcade::GetMap *map, arcade::WhereAmI *snake, char &direction)
{
  ...
  //std::cerr.write(reinterpret_cast<char*>(snake), (sizeof(uint16_t)*2)*(snake->lenght+1));
...
void	get_map(arcade::GetMap *map, arcade::WhereAmI *snake, char &direction)
{
  ...
  //std::cerr.write(reinterpret_cast<char*>(map),sizeof(uint16_t)*3 + sizeof(uint16_t)*((map->width * map->height)));
